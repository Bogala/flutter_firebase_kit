...
Ce projet est un starter kit Flutter préconfigurée avec une architecture Clean et modulaire, utilisant BLoC pour la
gestion d'état et des tests Gherkin (BDD).
...
# Lancer le générateur de code (injectable, retrofit, freezed, bdd_widget_test)
...
# Régénérer les flavors (ATTENTION: sauvegardez main.dart et app.dart avant)
...
├── main_*.dart     # Points d'entrée par flavor (dev, prod, test, etc.)
...
**Ne jamais importer directement des fichiers d'une couche inférieure sans passer par le module.**

## Conventions de code

### Fichiers `*_module.dart`

Chaque dossier contient un fichier `*_module.dart` qui exporte les éléments publics de ce dossier. Utilisez ces exports
pour contrôler la visibilité entre couches.
...
├── view/
│   ├── components/
│   │   ├── feature_button.dart      # Widget stateless
│   │   └── feature_form.dart        # Widget stateless
...
- Les routes sont injectées via des classes `*Module` qui implémentent `UIModule`

### Anti-Corruption Layer

Chaque entité du domaine doit avoir une factory `fromDto` pour convertir les DTOs en entités.
...
**Ne pas supprimer** les flavors `prod`, `dev` et `test`.

## Tests

### Tests BDD (Gherkin)

Les tests utilisent `bdd_widget_test`. Les fichiers `.feature` vont dans `test/gherkin/features/` et les steps dans
`test/gherkin/steps/`.

Steps pré-installées :

- `J'ai lancé l'application avec succès` - Lance l'app en mode test
- `L'application démarre depuis la route {'/'}` - Lance sur une route spécifique
- `Je redimensionne mon écran vers une largeur de {1900} et une hauteur de {1080}`

### Mocks API (Retrofit)

Placer les mocks dans `/mocks/api/` avec le format :
...
Ajouter les endpoints dans `lib/core/di/network/api_client.dart`. **Seuls les Repositories peuvent utiliser Retrofit.**
...
Format Angular : `<carte jira> - <type>(<portée>) : <résumé>`

Types : `build`, `ci`, `docs`, `feat`, `fix`, `perf`, `refactor`, `test`
Portées : `core`, `data`, `domain`, `ui`, `design`

Exemple : `LIS-123 - feat(ui) : ajouter écran de connexion`

## Fichiers générés

Les fichiers suivants sont générés automatiquement (ne pas modifier manuellement) :

- `*.g.dart` - Retrofit, JSON serialization
- `*.freezed.dart` - Freezed
- `lib/injection.config.dart` - Injectable

Ces fichiers sont exclus de l'analyse (`analysis_options.yaml`).
...
| `dio` + `retrofit`      | Client HTTP                   |
| `freezed`               | Immutabilité et unions        |
| `bdd_widget_test`       | Tests Gherkin                 |

## Configuration par environnement

Chaque flavor a sa propre configuration dans `lib/core/di/configuration/configuration_*.dart`. Modifier `apiBaseUrl` et
autres paramètres selon l'environnement.
